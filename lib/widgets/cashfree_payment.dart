...
class CashfreePayment extends StatefulWidget {
  ...
  const CashfreePayment({Key? key, required this.amount, required this.bookName})
      : super(key: key);
...
class CashfreePaymentState extends State<CashfreePayment> {
...
request.body = json.encode({ 
	"order_amount": widget.amount+40,// Order Amount in Rupees 
...
if (response.statusCode == 200) { 
  ...
  void handleCFPaymentError(CFErrorResponse errorResponse, String orderId) {
    ...
    Fluttertoast.showToast(
        msg: "Cashfree Payment Failed for Order ID: ${errorResponse.getMessage()}",
        toastLength: Toast.LENGTH_SHORT);
  }
